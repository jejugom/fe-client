...
const isPercent = computed(() => {
  const vals = sorted.value.map(d => Math.abs(Number(d.returnRate))).filter(n => Number.isFinite(n));
...
const dailySeries = computed<[number, number][]>(() =>
  sorted.value.map(d => {
    const pct = isPercent.value ? Number(d.returnRate) : Number(d.returnRate) * 100;
...
const cumulativeSeries = computed<[number, number][]>(() => {
  ...
  return sorted.value.map(d => {
    const rDecimal = isPercent.value ? Number(d.returnRate) / 100 : Number(d.returnRate);
...
const palette = {
  dailyStrong: '#2563eb',      // 파랑
  dailySoft: 'rgba(37,99,235,0.25)',
  cumStrong: '#16a34a',        // 초록
  cumSoft: 'rgba(22,163,74,0.25)',
  grey: '#cbd5e1'
};
...
const options = computed<any>(() => {
  ...
  return {
    ...
    xaxis: {
      type: 'datetime',
      tickAmount,
      labels: { datetimeUTC: false, format: 'yyyy-MM-dd', rotate: -15 },
      axisBorder: { show: false },
      axisTicks: { show: false }
    },
    yaxis: [
      {
        title: { text: '일일(%)', style: { color: isDaily ? palette.dailyStrong : palette.grey } },
      ...
      {
        opposite: true,
        title: { text: '누적(%)', style: { color: !isDaily ? palette.cumStrong : palette.grey } },
        min: yCum.value.min,
        max: yCum.value.max,
        tickAmount,
        labels: { formatter: (v: number) => `${v.toFixed(1)}%` }
    ...
    annotations: {
      yaxis: [
        {
          y: 0, yAxisIndex: 0,
          borderColor: '#c5ced7', strokeDashArray: 4,
          label: { text: '0%', style: { background: 'transparent', color: '#8a95a6' }, borderWidth: 0 }
        },
        {
          y: 0, yAxisIndex: 1,
          borderColor: '#c5ced7', strokeDashArray: 4,
...
<template>
  <div class="flex flex-col gap-3">
    <!-- 토글 버튼 -->
    <div class="inline-flex w-max items-center overflow-hidden rounded-xl border border-surface-200">
      <button
        class="px-4 py-2 text-sm"
        :class="mode === 'daily' ? 'bg-primary-50 text-primary-700 font-semibold' : 'text-surface-500 hover:bg-surface-50'"
        @click="mode = 'daily'"
      >
        일일수익률
      </button>
      <button
        class="px-4 py-2 text-sm"
        :class="mode === 'cumulative' ? 'bg-primary-50 text-primary-700 font-semibold' : 'text-surface-500 hover:bg-surface-50'"
...
.apexcharts-marker { opacity: 1 !important; }
</style>
