<template>
  <div
    :class="[
      'border-surface-200 flex h-31.5 w-full items-center justify-between gap-6 rounded-lg border bg-white p-4',
      $attrs.class,
    ]"
    v-bind="$attrs"
  >
    ...
    <div class="flex w-2/3 flex-col">
      <!-- 제목 -->
      <div
        :class="['text-primary-500 overflow-hidden text-lg font-semibold']"
        >{{ title }}</div
      >
      <!-- 내용 -->
      <div class="text-surface-500 mt-2 mb-1 flex flex-wrap gap-1 truncate">
        <span>
          {{ content }}
        </span>
      </div>
